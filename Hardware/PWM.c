...
uint8_t dt = 0;//存放波形占空比

/**
  * 函    数：PWM初始化，PB0输出PWM波
  * 参    数：无
  * 返 回 值：无
  */
...
//	GPIO_PinRemapConfig(GPIO_Remap_SWJ_JTAGDisable, ENABLE);		//将JTAG引脚失能，作为普通GPIO引脚使用
	...
	/*f=72M/(psc*arr)*/
	TIM_TimeBaseInitStructure.TIM_Period = (arr_0 - 1);					//计数周期，即ARR的值
	TIM_TimeBaseInitStructure.TIM_Prescaler = (psc_0 - 1);				//预分频器，即PSC的值
	...
	TIM_OCInitStructure.TIM_Pulse = dt;								//初始的CCR值
